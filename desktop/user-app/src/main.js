...
const launcherScript = path.join(rootDir, 'frontend', 'start_with_backend.py');
const frontendHost = process.env.FRONTEND_HOST || '127.0.0.1';
const frontendPort = process.env.FRONTEND_PORT || '8080';
const entryUrl = process.env.USER_ENTRY_URL || `http://${frontendHost}:${frontendPort}/`;
...
const startStack = () => {
	...
	env.SECRET_KEY = env.SECRET_KEY || 'desktop-user-secret';
	...
	stackProcess.on('exit', (code) => {
		if (code !== 0) {
			console.error(`[user-app] stack process exited with code ${code}`);
		}
	});
...
const bootstrap = async () => {
	try {
		...
		console.error('Failed to start user desktop app', err);
